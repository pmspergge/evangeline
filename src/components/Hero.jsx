...
export function Hero({ onCustomCakeClick }) {
  ...
  return (
    <section className="pt-16 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
      ...
      {/* Título Principal */}
      <h1 className="font-serif text-4xl sm:text-6xl text-[#1C1917] leading-tight mb-6">
        El arte de la repostería <br className="hidden md:block" />
        fina en <span className="text-[#B89855] italic">La Plata</span>
      </h1>

      {/* Descripción */}
      <p className="max-w-2xl mx-auto text-[#1C1917]/70 text-base sm:text-lg mb-10 font-light">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 mb-16">
        <button
          ...
          className="bg-[#1C1917] text-[#FAF7F2] px-8 py-4 rounded-sm flex items-center justify-center gap-2 hover:bg-[#1C1917]/90 transition-colors uppercase tracking-wider text-xs sm:text-sm font-medium"
        >
          Ver Menú & Pedir <ArrowRight size={18} />
        </button>
        <button
          onClick={onCustomCakeClick}
          className="border border-[#B89855] text-[#B89855] px-8 py-4 rounded-sm hover:bg-[#B89855]/10 transition-colors uppercase tracking-wider text-xs sm:text-sm font-medium"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={idx} className="bg-transparent border border-[#1C1917]/10 p-5 rounded-sm text-left flex flex-col gap-2 hover:border-[#B89855]/40 transition-colors">
            <item.icon className="text-[#B89855]" size={22} strokeWidth={1.5} />
            <div>
